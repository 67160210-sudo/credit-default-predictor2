...
if st.button("🔍 ประเมินความเสี่ยง", use_container_width=True):
    ...
    if model_loaded:
        ...
        logit = -6 + (0.0005 * balance) - (0.0001 * income) + (0.04 * debt_ratio)
        prob = 1 / (1 + np.exp(-logit))
        pred = 1 if prob > 0.5 else 0

    # 🔥 แสดง Debt Ratio ซ้ำใน Result
    st.markdown(f"""
<div class="ratio-box">
    <div class="ratio-top">
        <span>Debt Ratio</span>
...
""", unsafe_allow_html=True)
    ...
    # Result
    if pred == 0:
        st.markdown('<div class="res safe">', unsafe_allow_html=True)
        st.success(f"✅ ความเสี่ยงต่ำ ({(1-prob)*100:.2f}%)")
        st.write("✔ ลูกค้ามีแนวโน้มชำระหนี้ได้ดี")
        st.markdown('</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="res risk">', unsafe_allow_html=True)
        st.error(f"⚠️ ความเสี่ยงสูง ({prob*100:.2f}%)")
        st.write("✗ มีโอกาสผิดนัดชำระหนี้")
        st.markdown('</div>', unsafe_allow_html=True)

    st.write("---")
    st.write(f"💰 หนี้: {balance}")
    st.write(f"💵 รายได้: {income}")
    st.write(f"📊 Ratio: {debt_ratio:.2f}%")
